...
class FormatCommandTests(unittest.TestCase):
    ...
    def test_file_argument_formats_to_stdout(self) -> None:
        ...
        self.assertRegex(
            result.stderr,
            rf"Formatted 0/1 file\. 0/{fixture_loc(OUTPUT_FIXTURE)} LOC changed\. Done in (?:\d+ms|\d+\.\d{{3}}s)\.\s*$",
        )
    ...
    def test_dry_run_accepts_idempotent_file_and_rejects_unformatted_file(self) -> None:
        with copied_fixtures(INPUT_FIXTURE, OUTPUT_FIXTURE) as fixtures:
            ...
            self.assertRegex(
                ok_result.stdout,
                rf"Checked 1 file\. 0/{fixture_loc(OUTPUT_FIXTURE)} LOC will change\. Done in (?:\d+ms|\d+\.\d{{3}}s)\.\s*$",
            )

            bad_result = native_format("--dry-run", str(fixtures[INPUT_FIXTURE]))

            self.assertEqual(1, bad_result.returncode, msg=f"stdout:\n{bad_result.stdout}\n\nstderr:\n{bad_result.stderr}")
            self.assertIn("Formatting is required for 1/1 file.", bad_result.stdout)
            self.assertRegex(
                bad_result.stdout,
                rf"Formatting is required for 1/1 file\. [\d,]+/{fixture_loc(INPUT_FIXTURE)} LOC will change\. Done in (?:\d+ms|\d+\.\d{{3}}s)\.\s*$",
            )
    ...
    def test_diff_outputs_unified_diff_and_uses_dry_run_exit_codes(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="format_diff_", dir=build_dir) as temp_dir:
            ...
            self.assertIn("Formatting is required for 1/1 file. 1/1 LOC will change. Done in ", changed.stderr)
            ...
            self.assertRegex(clean.stderr, r"Checked 1 file\. 0/1 LOC will change\. Done in (?:\d+ms|\d+\.\d{3}s)\.\s*$")
        ...
        with tempfile.TemporaryDirectory(prefix="format_summary_", dir=TEST_TEMP_ROOT) as temp_dir:
            ...
            for mode in ((), ("-i",), ("-n",), ("--dry-run",), ("--diff",)):
                with self.subTest(mode=mode):
                    ...
                    action = "Formatting is required for" if checking else "Formatted"
                    change_verb = "will change" if checking else "changed"
                    self.assertRegex(
                        summary,
                        rf"^{action} 1/2 files\. 1/4 LOC {change_verb}\. Done in (?:\d+ms|\d+\.\d{{3}}s)\.\s*$",
                    )
                    self.assertEqual(
        ...
        for name, source, changed in cases:
            for mode in ((), ("-n",), ("--diff",)):
                with self.subTest(name=name, mode=mode):
                    ...
                    change_verb = "will change" if mode else "changed"
                    self.assertIn(f"stdin. {changed}/{len(source.splitlines())} LOC {change_verb}. Done in ", summary)
    ...
    def test_concurrency_one_preserves_file_list_output_order(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="format_order_", dir=build_dir) as temp_dir:
            ...
            self.assertRegex(result.stderr, r"Formatted 2/2 files\. 2/2 LOC changed\. Done in (?:\d+ms|\d+\.\d{3}s)\.\s*$")
    ...
    def test_in_place_formats_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="format_in_place_", dir=build_dir) as temp_dir:
            ...
            self.assertIn("Formatted 1/1 file. 1/1 LOC changed. Done in ", result.stdout)
    ...
    def test_style_file_merges_every_macro_category_with_parent(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="format_macro_category_inherit_", dir=build_dir) as temp_dir:
            ...
            source.write_text(
                ...
                "CHILD_SEMILESS()\n"
                "void Exercise(){\n"
                "if(enabled) PARENT_STATEMENT_PREFIX Run();\n"
                "CHILD_STATEMENT_PREFIX(values) { Run(); }\n"
    ...
    def test_ignore_file_skips_simple_directory_entries(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="format_ignore_", dir=build_dir) as temp_dir:
            ...
            self.assertEqual(0, result.returncode, msg=f"stdout:\n{result.stdout}\n\nstderr:\n{result.stderr}")
            self.assertIn("Checked 0 files. 0/0 LOC will change.", result.stdout)
