...
template <class T>
using OldValue ANNOTATION = T;
namespace aliases {

using OldPointer ANNOTATION = void*;
...
class [[nodiscard]] Comparison {
    template <
        typename traits::EnableIf<
            !traits::IsIntegral<Left>::value || !traits::IsPointer<Right>::value
        >::type* = nullptr
    >
    static Result Compare(traits::Null, Value* right) {
        return CompareValues(
            left_text, right_text, static_cast<Value*>(nullptr), right
        );
    }
};

Result Describe(const char* left_text, const char* right_text, const char* op) {
    return MakeResult() << " vs " << FormatValues(left, right);
}

class API_EXPORT [[nodiscard]] Helper {
    Helper(
        Event::Kind type, const char* file, int line, traits::String message
    );
...
Info* Register(
    ...
    return detail::RegisterInfo(new FactoryAdapter{traits::Move(factory)});
}
...
enum class PrefixedValues {
    First = ENUM_PREFIX Value,
    Second = ENUM_PREFIX Other,
};
...
struct EnumFragments {
    enum Nested {
        First = ENUM_PREFIX(Compute(1) + 2),
        Last,
    };
};

void LocalEnumFragments() {
    enum Local {
        First = ENUM_PREFIX Value,
        Last,
    };
...
API_EXPORT UNKNOWN_MODIFIER ns::String MixedDefinition(int value) {
    return Convert(value);
}
API_EXPORT UNKNOWN_MODIFIER(tag) ns::String MixedDeclaration(int value);

struct MixedModifiers {
    API_EXPORT UNKNOWN_MODIFIER inline ns::String Method(int value) {
        return Convert(value);
    }
