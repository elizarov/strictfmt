...
enum class PrefixedItems {ENUM_ITEMS First,ENUM_ITEMS Last};
...
template<class T> void TypeListCalls(){
...
DEFINE_ACTION(Work();Finish();,Startup)
...
ANNOTATION constexpr explicit ConstructorModifiers(int value) noexcept : value_(value) {}
template<class... Args> ANNOTATION explicit ConstructorModifiers(Args&&... args) noexcept(noexcept(Build(args...))) {}
template<class Value> ANNOTATION(reason(Nested(value))) explicit(false) ConstructorModifiers(Value value);
template<class Target> ANNOTATION ANNOTATION constexpr explicit operator Target() const {return Target{};}
};
