...
namespace данные{
struct Запись{int количество;};
void Обработать(Запись запись);
struct Источник{operator Запись()const;};
}
void Identifiers(){
Prepare();
ДоставкаНедоступнаДляАдреса(значение);
处理订单并发送通知(收件人,订单号);
𐐀𐐁𐐂𐐃𐐄𐐅𐐆𐐇𐐈𐐉𐐊𐐋𐐌𐐍𐐎𐐏𐐐𐐑𐐒𐐓(𐐀𐐁𐐂𐐃);
cafécafécafécafécafécafé(café);
данные::Обработать(данные::Запись{42});
}

// A joined spelling is measured anew.
void LiteralJoining(){
Prepare();
Use("012345678901234567890123456" "é");
Use("012345678901234567890123456" "e" "́");
Use("012345678901234567890123456" "👩🏽‍" "💻");
Use("012345678901234567890123456" "𝄞!");
Use("012345678901234567890123456\u00e9");
}
...
void LiteralPairs(){
auto text=first+"сообщение="+value+"𝄞="+other;
stream<<"сообщение="<<value<<"é="<<other;
}
...
Use(R"(Доставка недоступна
01234567890123456789012345678901234é)",x);
LongerUse(R"(0123456789012345678901234567890𝄞
Доставка)");
}
