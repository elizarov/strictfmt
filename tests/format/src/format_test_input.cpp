...
static_assert(true,"This diagnostic deliberately makes the static assertion exceed the line limit so its condition and message must be formatted on separate lines.");
}
...
struct Record { int value; int Read() const { return value; } };
...
template<int Size> struct Array { Pair values[Size]; };
template<int... Values>
...
template<int... Values> struct Holder {
Array<sizeof...(Values)> array;
Holder():array{{{Values,Values+1}...}} {}
};
template<int... Values> constexpr auto Conditional() {
...
void EmptyCompoundStatements(bool condition) { if (condition) { {}; } }
...
int total=0;
if(int left=1,right=2;left<right) total+=left+right;
if(int first(4),second{5};first<second) total+=first+second;
if(int& left=total,&right=total; &left==&right) ++total;
switch(int selector=1,increment=2;selector) { case 1: total+=increment; break; }
if(typedef int Number;Number{0}==0) ++total;
if(using Number=int;Number{1}==1) ++total;
return total;
}
}

namespace QualifiedConditionExpressions {
struct Flags { static constexpr int first=1,second=2; };
constexpr bool Evaluate() {
if constexpr(Flags::first & Flags::second) return false;
if constexpr(Flags::first && Flags::second) return true;
return false;
}
}
...
struct Empty {};
struct Value { Empty empty; int count; };
Value valuesWithAnIntentionallyLongNameToForceWrappingTheNestedInitializer[] = {{Empty{
...
Triple valuesWithAnIntentionallyExtendedNameToMakeTheWholeInitializerExceedTheColumnLimit{1 /* first */, 2 /* second */, 3 /* third */};
...
Triple lineComment{1,2,3, // trailing
};
Triple standaloneComment{1,2,3,
/* standalone */
};
int conditional[] = {
#if 1
...
enum class Flag : bool { Off [[deprecated]] = false, On = true };
enum Status : unsigned long { Old [[deprecated("use Current")]] = 0, Current = 1 };
enum class Character : char { A [[maybe_unused]] = 'a', B = 'b' };
enum Attributes { Repeated [[maybe_unused]] [[deprecated("use Current")]] = 2 };
...
void Several() {
const int firstValueWithAnIntentionallyLongName=1, secondValueWithAnIntentionallyLongName=2, thirdValueWithAnIntentionallyLongName=3, // final item on this source line
fourth=4;
...
struct Fields {int first=1, // field
second=2;};
}
...
namespace helpers { bool IsRetailPresented(int revision); }
bool Check(int revision, const Context& stores_context) {
    if (!helpers::IsRetailPresented(revision) &&
    ...
    (void)grouped;
    (void)pointer;
    if (revision > 0 /* inline at closer */) { return true; }
    if (revision > 0 // trailing line comment
    ) { return true; }
    if (revision > 0
...
template <typename T> struct Optional { Optional(T); };
template <typename Callback> auto Async(const char*, Callback callback) { return callback(); }
template <typename Callback> auto Visit(Callback callback) { return callback(); }
...
auto Work(int maybe_product_specific, int specifics_storage_client, int idempotency_token) {
    return Async("store product specific if needed",
        [&maybe_product_specific, &specifics_storage_client, idempotency_token]() -> Optional<InsuranceProductSpecificIdentity> {
            if (!maybe_product_specific) return InsuranceProductSpecificIdentity{};
...
template <typename... Callbacks> void Consume(Callbacks...) {}
void Guards() {
    Consume([] { return Visit([] { return 0; }); }
    ...
    int (*callbacks[])() = {+[] { return Visit([] { return 0; }); }
...
FORMAT_TOKEN_ITEM(global, +, (one, two)(three, four)) FORMAT_TOKEN_ITEM(adjacent, *, )
...
void Run(bool condition) {
 FORMAT_TOKEN_ITEM(local, +, (one, two)) FORMAT_TOKEN_STATEMENT(+, )
 if (condition) FORMAT_TOKEN_STATEMENT(+, (x, y)) else FORMAT_TOKEN_STATEMENT(*, (z))
 while (condition) FORMAT_TOKEN_STATEMENT(++, <>);
 for (; condition;) FORMAT_TOKEN_STATEMENT(/, [])
...
template<class T> void Default(T value FORMAT_PARAMETER_SUFFIX_CALL(T) = {}) { (void)value; }
...
struct Context { Data data; };
using Flag = bool;
template<class T> bool Read(const T& object, bool decltype(T::data)::* member) { return object.data.*member; }
template<class T> bool Named(const T& object, Flag decltype(T::data)::* member) { return object.data.*member; }
using Member = bool decltype(Context::data)::*;
typedef bool decltype(Context::data)::* TypedefMember;
struct Holder { bool decltype(Context::data)::* member; Flag decltype(Context::data)::* named; };
Flag decltype(Context::data)::* member = &Data::flag;
namespace constraints { template<class> concept Any = true; template<class, class> concept Matches = true; }
...
enum Choice { First };
auto ClassLambda() { return []() -> class Value { return {}; }; }
auto StructLambda() { return []() -> struct Record { return {}; }; }
auto UnionLambda() { return []() -> union Number { return {1}; }; }
auto EnumLambda() { return []() -> enum Choice { return First; }; }
namespace detail { class Value {}; }
auto Qualified() { return [](auto) -> class detail::Value { return {}; }; }
template<class T> auto Dependent() -> class T::Value { return {}; }
template<class T> auto DependentLambda() { return []() -> class T::Value { return {}; }; }
...
using InlineRecord = struct { int value; };
using InlineChoice = enum { FirstInline };
struct Owner { auto Method() -> class Value { return {}; } };
...
namespace memory { template<class T> struct Pointer {}; }
namespace future { template<class T> struct Shared {}; }
namespace values { template<class T> struct Optional {}; }
namespace tasks { template<class T> T Make(int) { return {}; } }
auto NestedCall() {
return tasks::Make<
future::Shared<values::Optional<memory::Pointer<int>>>>(0);
}
struct Key {};
struct Data {};
struct Construct {
...
void Iterate() {
for(const auto& [index, name, pairs] : std::vector<
std::tuple<int, int, std::vector<std::pair<int, int>>>
>{{1, 2, {{3, 4}}}}) {
(void)index; (void)name; (void)pairs;
}
}
template<class T> struct Never : std::false_type {};
template<class... Args> constexpr bool Fold() {
return !(Never<std::remove_const_t<std::remove_reference_t<Args>>>::value || ...);
}
}
...
namespace detail { struct Result {}; }
...
FORMAT_STATEMENT_DECLARATIONS(int first = 1; [[maybe_unused]] int second = first + 1;, int);
FORMAT_STATEMENT_DECLARATIONS([[maybe_unused]] Value value{1};, int);
FORMAT_STATEMENT_DECLARATIONS(Value value = MakeValue(); [[maybe_unused]] auto& reference = value;, int);
FORMAT_STATEMENT_DECLARATIONS([[maybe_unused]] auto [value] = MakeValue();, int);
FORMAT_STATEMENT_DECLARATIONS(int value; value = 1;, int);
FORMAT_STATEMENT_DECLARATIONS(Value value; value.value = 2;, int);
...
template<class T> T operator+(T value) const { return value; }
template<class T> bool operator==(T value) const { return value == T{}; }
template<class T> bool operator<(T value) const { return value < T{}; }
template<class T> T operator<<(T value) const { return value; }
template<class T> Transform& operator=(T) { return *this; }
...
template<class T> auto Relay(T value) const -> decltype(operator()(value)) { return operator()(value); }
};
...
template<> long Transform::operator+<long>(long value) const { return value; }
...
namespace geometry { struct Position {}; template<class T> struct Box {}; }
struct Point {
...
template<class Target, class Source> Target Convert(Source& source) { return source.operator Target(); }
...
constexpr auto splicedQuantity=1\
_quantity;
}
...
struct Methods { FORMAT_TYPE_TEMPLATE_HEADER(Convert) { return value; } };
...
template<class T> struct Values { template<class U> U Convert(U value); };
...
template<class Owner> struct EmptyMethods { FORMAT_TYPE_VOID_HEADER(Visit) {} };
...
namespace Details { struct Trace { Trace(const char*, int, int) {} }; }
...
int after_chain;
REGISTER_ITEM(value)->Configure(1); int after_terminated_chain;
struct Nested {API_EXPORT int bare; API_EXPORT("test") int arguments;};
#if ENABLE_ITEMS
API_EXPORT("test") int conditional;
#endif
}
...
void ControlBodies(Image& image) {
 ...
 if (ready) FOR_PIXELS(image,x) { Consume(x); } else Finish();
 while (ready) FOR_PIXELS(image,x) { Consume(x); }
 FOR_PIXELS(image,x) {}
 auto visit = [&] { FOR_PIXELS(image,x) { Consume(x); } };
 switch (mode) { case 1: FOR_PIXELS(image,x) { Consume(x); } break; }
#if ENABLE_PIXELS
 FOR_PIXELS(image,x) { Consume(x); }
#endif
}
#define VISIT_PIXELS(image) do { FOR_PIXELS(image,x) { Consume(x); } } while (false)
...
Packed::Packed() : first_value_with_a_long_name_(FirstValue()), second_value_with_a_long_name_(SecondValue()), computed_([] { Prepare(); return Get(); }()), last_(Get()) { Check(); Done(); }
Expanded::Expanded() : first_value_with_a_long_name_(FirstValue()), second_value_with_a_long_name_(SecondValue()), third_value_with_a_long_name_(ThirdValue()), computed_([] { Prepare(); return Get(); }()), last_(Convert(Get())) { Check(); Done(); }
struct Nested {
Nested() : computed_([] { Prepare(); return Get(); }()), // computed value
last_(Get()) { Check(); Done(); }
};
Empty::Empty() : computed_([] { Prepare(); return Get(); }()), // computed value
last_(Get()) {}
Compact::Compact() : computed_([] { Prepare(); return Get(); }()), last_(Get()) { Check(); Done(); }
...
template<class T> class InlineConstructor final {
public:
UNKNOWN_MODIFIER constexpr explicit InlineConstructor(T value) noexcept : value_(value) {}
};
template<class T> class SplitConstructor final {
public:
UNKNOWN_MODIFIER
...
void Function() {
    if (ready) {
#define FORMAT_LOCAL_CONTROL(value) if(value){First();Second();}
...
class Holder {
#define FORMAT_LOCAL_METHOD(Name) void Name(){First();Second();}
...
void Arguments() {
    Invoke(
        first,
#define FORMAT_LOCAL_LIST(value) First(value); Second(value);
        second,
        third
    );
    Finish();
...
void Cases(int value) {
    switch(value) {
        case 0:
#define FORMAT_LOCAL_CASE(value) switch(value){case 1:First();break;default:Second();break;}
...
void Configured() {
FORMAT_STATEMENT_PREFIX_DEFER { (void)0; };
FORMAT_STATEMENT_PREFIX_NAMED(other) { (void)0; }; // declaration terminator
FORMAT_STATEMENT_PREFIX_NAMED(empty) {};;
FORMAT_STATEMENT_PREFIX_WRAPPER FORMAT_STATEMENT_PREFIX_NAMED(nested) { (void)0; };
}
void Inferred() {
FORMAT_INFERRED_DEFER(first) { (void)0; };
FORMAT_INFERRED_DEFER(empty) {};
FORMAT_INFERRED_DEFER(commented) { (void)0; } /* before terminator */ ; // after terminator
}
void ControlBodies(bool ready) {
if(ready) FORMAT_STATEMENT_PREFIX_DEFER { (void)0; }; else (void)0;
if(ready) FORMAT_INFERRED_DEFER(cleanup) {}; else (void)0;
while(ready) FORMAT_STATEMENT_PREFIX_WRAPPER FORMAT_STATEMENT_PREFIX_DEFER {};
for(int i=0;i<2;++i) FORMAT_STATEMENT_PREFIX_DEFER {};
do FORMAT_INFERRED_DEFER(cleanup) {}; while(ready);
if(ready) while(ready) FORMAT_STATEMENT_PREFIX_DEFER {}; else (void)0;
}
void Nested() {
...
#define FORMAT_DEFERRED_ACTION() FORMAT_STATEMENT_PREFIX_DEFER { (void)0; };
