...
void LongSingleMacroBodyWithAHeaderAndArgumentThatCannotFitOnOneLine() {
    MANY_STEPS(argument_with_a_deliberately_long_descriptive_name)
}
void MultipleMacros() {
    MANY_STEPS(first)
    MANY_STEPS(second)
...
template <class module, class import = int>
struct Values {
    module first;
    import second;
};
...
int Read() {
    ...
    auto sum =
        [module = module, import = import, final = 3, override = 4] { return module + import + final + override; };
    return sum();
}
template <class... T>
int CapturePack(T... module) {
...
struct Config {
    const Config& operator[](const char*) const;
    template <class T>
    T As() const;
};
...
struct Owner {
    Owner(const Config&, const Config&);

    int storage;
};

Owner::Owner(const Config& config, const Config&) :
    storage(Make<types::Storage>(config["key"].As<types::Optional<types::List<types::Text>>>())) {}
...
template <class T>
struct InlineItem {
    ~InlineItem<T>() = default;
};

void Destroy() {
    Item<int> item;
    InlineItem<int> other;
}
...
constexpr bool Compare() { return Condition; }
template <int N>
constexpr int Bits() { return N; }
...
auto Make(std::integer_sequence<unsigned, Indices...>) {
    return std::tuple<std::optional<std::tuple_element_t<Indices, T>>...>();
}
...
template <class Initial, class Base>
struct Combined {
    template <class T>
    using InitialArg = typename std::conditional<std::is_scalar<T>::value, T, const T&>::type;
    ...
    template <
        ...
        typename std::enable_if<
            std::conjunction<
                std::is_convertible<Initial, Action<void(InitialArg<Args>...)>>,
                std::is_convertible<Base, Action<R(Args...)>>
            >::value,
            int
        >::type = 0
    >
...
template <auto Settings, class... Types>
struct Variant {};

struct Data {
    static constexpr int kSettings = 1;
};
...
auto Parse(Node node) {
    return node.template As<
        std::optional<Variant<&::TemplateArgumentNameRegressions::Data::kSettings, Primitive<First>, Primitive<Second>>>
    >();
}
...
template <class Type>
using negation = std::negation<Type>;

template <typename InitialAction, typename Base>
class Combined {
private:
    template <typename T>
    using InitialActionArgType = T;
...
public:
    template <
        ...
        typename std::enable_if<
            conjunction<
                negation<std::is_convertible<InitialAction, OnceAction<void(InitialActionArgType<Args>...)>>>,
                std::is_convertible<InitialAction, Action<void(InitialActionArgType<Args>...)>>,
                std::is_convertible<Base, OnceAction<R(Args...)>>
            >::value,
            int
        >::type = 0
    >
...
auto Debts() {
    std::vector<Task<std::tuple<std::optional<First>, std::optional<Second>, std::optional<Third>>>> values;
    return values;
}
auto Results() {
    std::vector<
        Task<std::tuple<std::vector<First>, std::string, std::optional<std::string>, std::optional<std::string>>>
    > values{};
    return values;
}
...
struct Rows {
    template <class T>
    T AsContainer(int) { return {}; }
};

struct Connection {
    Rows Execute(int) { return {}; }
};

auto Fetch(Connection connection) {
    auto rows = connection
        .Execute(0)
        .AsContainer<std::vector<std::tuple<First, std::optional<Second>, std::optional<Third>, std::optional<Third>>>>(
            0
        );
    return rows;
}
...
template <template <class...> class Template, class Bound>
struct Bind {
    template <class... Args>
    using First = Template<Bound, Args...>;
};
...
template <class... T>
using Dependencies = Dependency<T...>;
template <class... T>
using Wrapped = Wrapper<T...>;
template <template <class...> class... T>
using Fused = Fusion<T...>;

}
template <template <class...> class T, class U>
using Bound = Bind<T, U>;
...
template <class InitialAction, class Base>
struct Combined {
    template <typename T>
    using InitialActionArgType = typename std::conditional<std::is_scalar<T>::value, T, const T&>::type;
...
Number Twice(Number value) { return value * 2; }
...
int Read(Prefix value) { return value.value; }
...
#ifdef FORMAT_GUARDED_NAMESPACE_ENABLED
namespace [[deprecated]] Attributed {
...
#define FORMAT_SEMILESS_EQUAL(Type)                                                                \
    inline bool operator==(const Type& a, const Type& b) noexcept {                                \
        return true FORMAT_TOKEN_COMPARE(FORMAT_CONTINUATION_COMPARE, first) FORMAT_TOKEN_COMPARE( \
            FORMAT_CONTINUATION_COMPARE, second                                                    \
        );                                                                                         \
    }
FORMAT_SEMILESS_EQUAL(Value)
constexpr bool Same(int left, int right) {
    return true FORMAT_CONTINUATION_COMPARE(left, right) FORMAT_CONTINUATION_TRUE_TAIL;
}
...
constexpr bool Conditional(bool condition, int left, int right) {
    return condition ? true FORMAT_CONTINUATION_COMPARE(left, right) : false;
}
...
void Statements(int& value) {
    if (true FORMAT_CONTINUATION_TRUE_TAIL) {
        value = value FORMAT_CONTINUATION_ADD(1);
    }
    while (value FORMAT_CONTINUATION_ADD(1) < 3) {
        ++value;
    }
}
...
FORMAT_DECLARATOR_MODIFIER(
    tag
) UNKNOWN_MODIFIER ANOTHER_MODIFIER inline ns::String Inline(int value) { return Convert(value); }
