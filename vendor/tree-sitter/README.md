...
The regeneration tool downloads the pinned host tree-sitter CLI under `build/` when no `--tree-sitter-cli` path is provided. After the stock generator finishes, it compacts only the C spelling of the monolithic parse tables: symbolic enum references and identity `STATE`/`ACTIONS` wrappers become their numeric values, and unnecessary table whitespace is removed. The numeric tables, structures, dimensions, ABI, generated parser header, and upstream runtime remain unchanged. Regeneration rejects tables that violate the upstream runtime limits or a compacted `parser.c` larger than 50,000,000 bytes.
