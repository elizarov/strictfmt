...
When a mandatory block flushes pending tokens from inside a list, the break model preserves the selected layout of every enclosing matched-delimiter group and colon-prefixed list up to the containing block. Delimiter groups receive virtual closers so the solver records their compact-or-split choices before any opener is emitted. A colon-prefixed list needs no virtual closer: if its prefix-list node selects full-split form, deferred emission continues breaking its top-level commas until printing leaves the list. The printer therefore follows recorded choices across the block boundary instead of inferring an outer layout after the prefix has already been printed.
