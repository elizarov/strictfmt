...
When a mandatory block flushes pending tokens from inside a list, the break model preserves the selected layout of every enclosing matched-delimiter group and colon-prefixed list up to the containing block. `FormatListContinuation` supplies virtual closers and retains selected list indentation, so the solver records compact-or-split choices before any opener is emitted. Delimiter lookup uses direct ownership, so a nested call cannot supply the enclosing list’s opener. A colon-prefixed list needs no virtual closer: if its prefix-list node selects full-split form, deferred emission continues breaking its top-level commas until printing leaves the list. The printer therefore follows recorded choices across the boundary instead of inferring an outer layout after the prefix has already been printed. The terminal block opener also carries its selected render base into the brace frame, with macro continuation indentation removed from the structural base. Enclosing list-item indentation governs deferred separators; it must not replace the opener’s own base when a nested chain or header introduces another indentation level.
...
Before a mandatory block is printed, `FormatChainContinuation` builds the containing source item's break model and identifies the operand containing its opener. It records the enclosing chain's break opportunities when that operand cannot expand in compact form, including the first operand of a binary chain with one operator. Receiver expansion remains allowed for member chains, and unrelated sibling chains acquire no constraint. Token-free directives also require enclosing chains to split, including when they interrupt the final operand; conditional regions retain their separate branch handling. Each formatted segment receives those requirements, the original chain base indentation, and its flat-indentation policy, so it can select and emit the same split form on both sides of the boundary. Nested ternaries carry their colon breaks, while parenthesized comma expressions use the corresponding deferred-list state.

Directive analysis retains continuation layout for ordinary operators as well as uniform chains. For a separator starting a resumed line, the builder moves its emitted token into the following operand and leaves the original operator entry context-only. This makes its width and binding part of the same model consumed by solving and emission; comments around the leading separator preserve their order. A continued owner/value prefix with no remaining prefix break is a sequence, and an absent return type cannot introduce a function-signature break.
