...
The solver receives a `FormatBreakModel` projected from the persistent layout tree for one formatted segment and returns a `FormatBreakSolution`, which records the selected `FormatBreakChoice` and render base indentation for each selected structural break-model node. Recording every structural render base is necessary because a compact parent may contain an earlier child that changes the current indentation before a later child is emitted. The layout lowerer must record exactly the selected choices and render bases; it must not re-run local layout decisions or infer hidden choices from child nodes.
...
A declaration type and its declarator form an independent outer break node. Grammar type roles distinguish types from declaration modifiers. Its type subtree is structurally deeper, so the boundary is cheaper than every break inside the type without a surcharge. Pointer and reference wrappers are partitioned with the type, assignment tails remain outside this grouping, and callable declarations retain the ordinary function-signature model and its prefix surcharge.
...
When a mandatory boundary interrupts a list, its cost region retains the actual closer as a context-only reference to the complete model. `FormatListContinuation` records the selected item and closer indentation on the list owner. Boundary queries use lexical ownership; crossing a block or delimiter never consumes that placement. Colon-prefixed lists use the same owner placement without a closer. Each block opener retains its own selected render base, with macro continuation indentation removed from the structural base; an enclosing list's item indentation does not replace it.

Preprocessor list planning runs before flushing the buffered prefix. Delimited and colon-prefixed lists retain the solver-selected item indentation across definitions and other directives. Resuming a function or type body restores its structural owner indentation, even when a directive ended the header in the preceding segment.
...
Before a mandatory boundary is planned, `FormatChainContinuation` reads the retained complete item model and identifies the containing operands. A chain whose compact form cannot contain that boundary requires its uniform split form; expandable receivers and unrelated siblings remain unconstrained. Each operator references its complete chain owner's placement, which stores the selected base and flat-indentation policy once. Token-free directives also constrain the final operand; conditional regions retain their branch handling. Nested ternaries carry their colon breaks, and parenthesized comma expressions use list-owner placements.

Directive analysis retains continuation layout for ordinary operators as well as uniform chains. For a separator starting a resumed line, projection moves its emitted token into the following operand and leaves the original operator entry context-only. This makes its width and binding part of the same model consumed by solving and emission; comments around the leading separator preserve their order. A continued owner/value prefix with no remaining prefix break is a sequence, and an absent return type cannot introduce a function-signature break.

Block-chain lookahead skips exact model construction when the suffix after the complete block has no possible chain operator. Operators inside the block cannot belong to an enclosing chain because the body is a separate syntax subtree. When the closing brace is not represented, the check conservatively scans from the opener instead.
