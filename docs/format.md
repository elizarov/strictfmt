...
Optional breaks within a [formatted segment](glossary.md#formatted-segment) are chosen by [Break Selection](#break-selection):
...
Lambda [headers](glossary.md#callable-header) follow function-header placement; an owner prefix acts like a return-type prefix. Captures belong to the callable prefix and use [list layout](#lists), independently of parameters. Prefix breaks are structurally deeper than parameter breaks, so equal-cost wrapping splits parameters first.
